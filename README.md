# 🗄️ WorkByte Database

Database system for **WorkByte**, supporting core application data such as users, jobs, and proposals.

---

## 📌 Overview

This repository manages the database layer of WorkByte, including:

* Schema definitions
* Enum types
* Relationships between entities
* Data consistency with backend services

---

## 🏗️ Tech Stack

* **Database**: PostgreSQL
* **Containerization**: Docker

---

## 🐳 Development

All development is done inside the Docker container environment.

---

## 🔗 Related Repositories

* Backend: https://github.com/hwasyui/WorkByte-BACKEND
* Frontend: https://github.com/hwasyui/WorkByte-FRONTEND

---

## 👥 Team Members & Commit Codes

| Code  | Name           | GitHub                            |
| ----- | -------------- | --------------------------------- |
| [ASW] | hwasyui        | https://github.com/hwasyui        |
| [IKP] | tannpsy        | https://github.com/tannpsy        |
| [SKF] | sarahkimberlyy | https://github.com/sarahkimberlyy |

---

## 📝 Notes

* Keep schema aligned with backend models
* Use consistent naming conventions

---

## 📄 License

This project is for academic (capstone) purposes.
